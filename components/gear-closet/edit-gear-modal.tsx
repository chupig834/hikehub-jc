...
export default function EditGearModal({ item, onClose }) {
  const [name, setName] = useState(item.name ?? "")
  const [link, setLink] = useState(item.link ?? "")
  const [weight, setWeight] = useState(item.weight ?? "")
  const [unit, setUnit] = useState(item.unit ?? "oz")
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`/api/gear/${item._id}`, {
        method: "PUT",
      ...
      <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
        <div className="bg-gray-50 rounded-lg shadow-lg w-full max-w-xl px-16 py-9 pointer-events-auto">
          <div className="text-center mb-6">
            <h2 className="text-2xl font-bold">Edit Gear Closet Item</h2>
          </div>
